...
@Namespace("/deal")
public class SavedealAction implements SessionAware {
	private Map<String,Object> session;
	private DealService service;
	private Deals deals;
	
	@Action(value="savedeal",results={@Result(name="success",type="redirectAction",params={"actionName","deal"})})
	public String savedeal(){
		
		Dealtype dealtype = deals.getDealtype();
		service.saveDealtype(dealtype);
		User user = (User) session.get("user");
		Contact contact = (Contact) session.get("contact");
